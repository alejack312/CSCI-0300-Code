...
void copy_mappings(x86_64_pagetable* dst, x86_64_pagetable* src) {
    ...
    // Iterate over all virtual addresses in the source page table
    for (vmiter it(src); it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
        // If the virtual address is present in the source page table
        if (it.present()) {
